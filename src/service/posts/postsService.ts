...
export const validatePostInfo = async (postId: number): Promise<PostValidationResponse> =>{
    const postInfo = await db.select().from(Posts).where(eq(Posts.id, postId)).then((posts)=> posts[0]);
